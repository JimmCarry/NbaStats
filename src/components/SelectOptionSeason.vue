<template>
    <div class="container">
        <div @click="listActive = !listActive; $emit('changeYear', year)" class="selector" :class="{ 'active': listActive}">
            <h1>{{ year }}</h1>
            ...
            <ul v-if="listActive" class="list">
                <li @click="year = season.value" v-for="season in seasons" :key="season.id" class="options">
                    <p>{{ season.value }}</p>
                </li>
            </ul>
        </div>        
